                                ...
                                <form class="add-to-cart-form" method="POST" action="{{ route('public.cart.add-to-cart') }}">
                                    ...
                                    <div class="ps-product__shopping">
                                        ...
                                        @if (EcommerceHelper::isQuoteOnlyEnabled($product->id))
                                            <button 
                                                class="ps-btn ps-btn--black add-to-quote-button" 
                                                type="button" 
                                                data-bb-toggle="add-to-quote"
                                                data-url="{{ route('public.quote.add-to-quote') }}"
                                                data-product-id="{{ $product->id }}"
                                                @if ($product->isOutOfStock() || (!$productVariation && $product->is_variation)) disabled @endif
                                            >
                                                <img src="{{ Theme::asset()->url('img/quotation-icon.png') }}" alt="{{ __('Add To Quote') }}" style="width: 16px; height: 16px; vertical-align: middle; margin-right: 5px;"> {{ __('Add to Quote') }}
                                            </button>
                                        @else
                                            @if (EcommerceHelper::isQuoteEnabled($product->id))
                                                <button 
                                                    class="ps-btn ps-btn--outline add-to-quote-button" 
                                                    type="button" 
                                                    data-bb-toggle="add-to-quote"
                                                    data-url="{{ route('public.quote.add-to-quote') }}"
                                                    data-product-id="{{$product->id}}"
