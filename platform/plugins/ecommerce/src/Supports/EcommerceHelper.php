...
class EcommerceHelper
{
    ...
    public function isQuoteEnabled(int|string $productId): bool
    {
        return Product::query()
        ->where('id', $productId)        
        ->value('show_add_to_quote');
    }

    public function isQuoteOnlyEnabled(int|string $productId): bool
    {
        return Product::query()
        ->where('id', $productId)        
        ->value('is_request_for_quote');
    }
    ...
    public function registerThemeAssets(): void
    {
        ...
        Theme::asset()
            ->container('footer')
            ->add('add-to-quote-js', 'vendor/core/plugins/ecommerce/js/add-to-quote.js', [], version: $version);
